...
/**
 * Factory object responsible for creating {@link Collector} operators. A collector operator will be
 * requested for each output file of workflow tasks when task command execution is successfully completed.
 * Therefore, factory might implement singleton collector object or create a new collector object
 * each time when requested depending on the implementation, e.g. thread safety.
 */
@DeveloperApi
public interface CollectorSupport {

    /**
     * Returns a collector operator with specific plugin context and operator context that is created
     * and initialized by SeqsLab workflow engine.
     * @param pluginContext Plugin context returned from plugin initialization
     * @param operatorContext Operator context containing configuration parameters and task outputs
